...
export class ERPDialog {
    static logger = ServerHandler.getLogger();

    public static init(library: Library) {
        library.dialog("ERP", ERPDialog.getDialog()).triggerAction({
            matches: ["BISM:ERP", "ERP"]
        });
    }
    private static getDialog(): IDialogWaterfallStep[] {
        return [
            (session: Session, args) => {
                ERPDialog.logger.info("Entered : ERPDialog :[BISM:ERP] ")
                session.dialogData.callId = args;
                    session.send(BIBotConnector.channelDataMessage(session, PorpertiesUtil.getJSONProperties(session, "bismLanguage.SAPDialog.insure_protection_erp_production"), false, "", "", "", "ignoringInput").toMessage());
                    var msg = BIBotConnector.channelDataMessage(session, PorpertiesUtil.getJSONProperties(session, "bismLanguage.SAPDialog.erp_prompt_msg"), false, PorpertiesUtil.getJSONProperties(session, "bismLanguage.askMoreInformationDialog.prompt_choice_result"), "", "", "expectingInput").toMessage();
            ...
            (session: Session, results: IDialogResult<IFindMatchResult>, args: any) => {
                if (results.response.index == 0) {
                    ...
                    session.privateConversationData.configItemFlag = PorpertiesUtil.getJSONPropertiesWithoutSession("bi_configItems.ERPSAPDialog");
                ...
                ERPDialog.logger.info("Exit : ERPDialog :[BISM:ERP] ")
            }
