...
export class AskMoreInformationDialog {
    static logger = ServerHandler.getLogger();

    public static init(library: Library) {
        library.dialog("ask.MoreInformation", AskMoreInformationDialog.getDialog()).triggerAction({
            matches: ["BISM:ask.MoreInformation", "askMoreInformation", "more Information", "information"]
        });
    }
    ...
    private static getDialog(): IDialogWaterfallStep[] {
        return [
            (session: Session, args: any) => {
                AskMoreInformationDialog.logger.info("Entered : AskMoreInformationDialog :[BISM:ask.MoreInformation] ")
                session.dialogData.callId = args.callId;
                session.dialogData.ConfigItemFlag = args.ConfigItemFlag;
                session.privateConversationData.configItemFlag = args.ConfigItemFlag;
            ...
            (session: Session, results: IDialogResult<IFindMatchResult>, args: any) => {
                if (results.response.entity == undefined || results.response.entity == " ") {
                    ...
                    AskMoreInformationDialog.logger.info("Exit : AskMoreInformationDialog :[BISM:ask.MoreInformation] ")
                }
            },
